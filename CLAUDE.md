...
| `src/views/map-view.tsx` | **Maps** (`VIEW_MAP`, `map` icon): a spatial drawing canvas per project (pan/zoom SVG). Zones are polygons drawn by clicking vertices (close by clicking the first vertex, ≥3 pts, live dashed fill preview); selecting a zone shows draggable vertex handles + a horizontal context-menu panel (`ZonePanel`: grip-move whole zone, "Associate with…" a location via `RecordSuggestModal` — creates a node pinned inside the zone at the menu-open point, color picker, transparency slider + reset, node-size select `NODE_SIZE_PRESETS`, lock toggle, delete). Outline color is a `darker()` shade of the fill. Right-click empty space opens a global menu (Draw zone; Import image + Waypoints are stubs). Persists to `Maps/<Project> Map.json` (JSON, debounced). **Phase-2 pending**: nested per-location maps on node double-click, sublocation wheel on node click, background-image import into `Maps/Images`, the waypoints (location-graph) mode, locked-image edge behavior |
