...
- [ ] Further quest layout polish (purpose-built visual layout beyond the field list) if the current form proves insufficient
...
- [x] Field-aware drops: pairs with a dedicated field open a menu at the drop point offering to fill it — character↔quest: "Add as quest giver"; session↔quest: "Set as received session" or "Completed/Abandoned/Failed in this session" (sets outcome + session); event↔session: "Link to this session" — these write to the note that owns the field regardless of drag direction; the generic "Add/Remove relationship…" is always the last option (dragged note declares); already-satisfied fills are hidden; a single remaining option acts immediately without a menu — `src/views/graph-view.tsx`
