...
- [x] Sublocations: a "New sublocation" header action on a Location's page creates a child Location that declares `sublocation of` back to its parent (`SUBLOCATION_REL` in types.ts — a plain relationship, not a separate frontmatter field; the identifier is what makes a location a sublocation, so it also works typed by hand) — otherwise a complete, independent Location entity like any other; the new page opens with Back returning to the parent — `src/types.ts`, `src/project.ts` (`connectTo.relType`), `src/views/entity-view.tsx`
...
- [x] Sublocations layer: locations declaring `sublocation of` leave the locations row and form a grid right under it — one cluster per parent (nested sublocations flatten under their rooted ancestor), 4 wide, wrapping to more rows (5th starts row two, 9th row three, …), centered under the parent; clusters sharing a grid row sweep apart, and the grid rows are real layers (bands/edge routing/checker apply) — `src/graph/layout.ts` (`SUB_COLS`)
...
- [ ] Search + filter for the graph: search by name highlights matching nodes (dim the rest, like selection dimming); filter with tick-able criteria and two modes toggled by an icon — eye-dashed = dim out what's filtered away, eye-closed = hide it completely
...
- [x] Field-aware drops: pairs with a dedicated field open a menu at the drop point offering to fill it — character↔quest: "Add as quest giver"; session↔quest: "Set as received session" or "Completed/Abandoned/Failed in this session" (sets outcome + session) — these write to the note that owns the field regardless of drag direction; the generic "Add/Remove relationship…" is always the last option (dragged note declares); already-satisfied fills are hidden; a single remaining option acts immediately without a menu — `src/views/graph-view.tsx`
